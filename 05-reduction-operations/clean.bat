@echo off
echo === OpenMP Reduction Operations Clean Script ===

:: Check for open processes
echo Checking for processes that might lock files...
taskkill /f /im OpenMP_ReductionOperations.exe 2>nul
...
if exist benchmark_results.txt del benchmark_results.txt
if exist validation_results.txt del validation_results.txt
