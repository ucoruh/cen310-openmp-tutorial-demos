...
echo === OpenMP Reduction Operations Run Script ===
echo.
...
echo   --reduction TYPE      Run specific reduction type (sum, product, min, max, custom, all)
echo   --help                Show this help message
echo.
echo Examples:
echo   run.bat --debug --verbose
echo   run.bat --release --benchmark --threads 8
echo   run.bat --reduction custom --threads 4
exit /b 0
...
set "EXE_PATH=build\%CONFIG%\OpenMP_ReductionOperations.exe"
...
if not "%REDUCTION_TYPE%"=="all" set ARGS=%ARGS% --reduction %REDUCTION_TYPE%

echo Running %CONFIG% configuration with %OMP_NUM_THREADS% threads...
if not "%ARGS%"=="" echo Mode: %MODE%
if not "%REDUCTION_TYPE%"=="all" echo Reduction type: %REDUCTION_TYPE%
echo.
