...
echo === OpenMP Synchronization Run Script ===
echo.
...
echo   --sync TYPE           Run specific synchronization type (critical, atomic, ordered, barrier, nowait, all)
echo   --help                Show this help message
echo.
echo Examples:
echo   run.bat --debug --verbose
echo   run.bat --release --benchmark --threads 8
echo   run.bat --sync critical --threads 4
exit /b 0
...
set "EXE_PATH=build\%CONFIG%\OpenMP_Synchronization.exe"
...
if not "%SYNC_TYPE%"=="all" set ARGS=%ARGS% --sync-type %SYNC_TYPE%

echo Running %CONFIG% configuration with %OMP_NUM_THREADS% threads...
if not "%ARGS%"=="" echo Mode: %MODE%
if not "%SYNC_TYPE%"=="all" echo Synchronization type: %SYNC_TYPE%
echo.
...
:: Özel çıkış kodu 999'u kontrol et (menüden 0 seçildiğinde)
if %ERRORLEVEL_BACKUP% EQU 999 (
    echo.
    echo Exiting program as requested...
    exit /b 999
)
...
if %ERRORLEVEL_BACKUP% EQU 0 (
    ...
    echo.
    echo Program exited with error code: %ERRORLEVEL_BACKUP%
)
