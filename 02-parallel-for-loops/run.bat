...
echo === OpenMP Parallel For Loops Run Script ===
echo.
...
set "EXE_PATH=build\%CONFIG%\OpenMP_ParallelForLoops.exe"
...
set ARGS=
if /i "%MODE%"=="verbose" set ARGS=--verbose
if /i "%MODE%"=="quick" set ARGS=--quick
if /i "%MODE%"=="benchmark" set ARGS=--benchmark
